...
pub const TASK_COMMANDS: &[CommandHelp] = &[
    ...
    CommandHelp {
        name: "task run-all",
        usage: "cx task run-all [--status pending] [--mode sequential|mixed|parallel] [--backend-pool codex,ollama] [--backend-cap backend=limit] [--max-workers N] [--fairness round_robin|least_loaded] [--halt-on-critical|--continue-on-critical] [--json|--text]",
