...
- CX role: orchestration/telemetry surface only, not kernel implementation
...
TurboQuant is out of scope for core CX command execution until a backend implementation exists.
...
- wiring speculative flags into stable CX user commands without backend support
...
- JSON artifact shape for later CX telemetry compatibility

## Planned CX Capability Surface

If backend work succeeds, CX should expose only capability/config:
...
No user-facing CX flag should become stable before:
