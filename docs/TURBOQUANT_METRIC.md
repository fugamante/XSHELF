...
Status: active
Scope: normalized CX-facing metric names across `llama.cpp` and `MLX`
...
CX should not pretend they are the same field.

## Normalized Field Model

CX-facing metric groups should be:
...
CX may compare backends on:

- correctness
- throughput
- wall time

CX may present memory side-by-side only if it labels the metric kind explicitly.
