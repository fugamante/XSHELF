...
\begin{itemize}
  \item Unexpected fallback: \texttt{./bin/xshelf where <cmd>} and confirm \texttt{execution\_path} via \texttt{./bin/xshelf version}.
  \item Schema command fails: inspect \texttt{./bin/xshelf quarantine list}, then \texttt{./bin/xshelf replay <id>}.
  \item Budget clipping surprises: check \texttt{./bin/xshelf budget} and \texttt{./bin/xshelf trace}; reduce capture scope or adjust budgets intentionally.
